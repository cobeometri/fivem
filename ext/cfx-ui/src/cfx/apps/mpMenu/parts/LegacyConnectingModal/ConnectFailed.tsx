...
import {
  ...
  useRenderedFormattedMessage,
} from "../../utils/messageFormatting";
import { Image } from "lr-components";
...
export const ConnectFailed = observer(function ConnectFailed(
  props: ConnectFailedProps
) {
  const { state, server, onClose = noop } = props;

  const { extra, extraActions, title = "#Servers_ConnectFailed" } = state;
...
function useServiceStatus(
  showStatusAnyway: boolean | undefined | null
): { title: string; body: string } | null {
  const PlatformStatusService = usePlatformStatusService();

  const { level } = PlatformStatusService;
...
const StateExtraAction = observer(function StateExtraAction(props: {
  action: string;
  server: IServerView;
}) {
  ...
  return <TextBlock typographic>{actionx}</TextBlock>;
});
  ...
  return (
    <Flex repell fullWidth centered gap="large">
      <Flex vertical centered>
        <Avatar size="large" url={streamerMode ? null : userAvatarURL} />
...
function ConnectivityIcon({
  ...
  const { icon, color } = iconsMap[state];
