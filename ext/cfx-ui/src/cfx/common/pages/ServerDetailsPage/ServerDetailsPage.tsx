...
export const ServerDetailsPage = observer(function Details(
  props: ServerDetailsPageProps
) {
  const {
    server,
    onScroll,
  ...
  const isCompleteServerLoading =
    shouldShowCompleteServerLoading &&
    ServersService.isServerLoading(
      server.id,
      ServerViewDetailsLevel.MasterListFull
    );
  ...
  const countryTitle = useServerCountryTitle(
    server.locale,
    ...
    <Page showLoader={isCompleteServerLoading}>
      <Island grow className={rootClassName}>
        <Scrollable onScroll={onScroll}>
          {hasBanner && <div className={s.banner} style={bannerStyle} />}
                        ...
                        <Flex gap="xlarge">
                          <Flex>
                            <div ref={scrollTrackingRef as any}>
                              <ServerConnectButton
                                server={server}
                                elementPlacement={ElementPlacements.ServerPage}
                              />
                            </div>
...
const Warning = observer(function Warning({ server }: { server: IServerView }) {
  ...
  const EOLLinkText = useL10n("#ServerDetail_EOLWarning2_Link", {
    link: EOL_LINK,
  });
  const EOSLinkText = useL10n("#ServerDetail_SupportWarning2_Link", {
    link: EOS_LINK,
  });
  ...
  if (isServerEOS(server)) {
    ...
    const type: React.ComponentProps<typeof InfoPanel>["type"] =
      currentUserIsOwner ? "warning" : "default";
  ...
  const icon = server.private ? <BsLockFill /> : <BsExclamationTriangleFill />;
