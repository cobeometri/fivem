...
constexpr const char kDefaultServerList[] = "https://content.grandrp.vn/server/ingress";
...
void GameServer::ProcessServerFrame(int frameTime)
{
...

	{
		...
		m_clientRegistry->ForAllClients([&](fx::ClientSharedPtr client)
		{
			...
			if (peer)
			{
				...
				if (!lm || fx::AnyCast<bool>(lm) != lockdownMode || !ss || fx::AnyCast<uint8_t>(ss) != syncStyle || !lf || (m_serverTime - fx::AnyCast<uint64_t>(lf)) > 1000)
				{
	...
	if (msec() >= m_nextHeartbeatTime)
	{
		...
		{
			auto var = m_instance->GetComponent<console::Context>()->GetVariableManager()->FindEntryRaw("sv_licenseKey");
			...
			if (var && !var->GetValue().empty())
			{
				...
				auto json = nlohmann::json::object({ { "port", m_instance->GetComponent<fx::TcpListenManager>()->GetPrimaryPort() },
				{ "listingToken", var->GetValue() },
